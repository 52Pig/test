...
# 上面的简单描述信息并没有什么卵用啊，咱们得再细一点分析下数据啊。
# 看看每个/多个 属性和最后的Survived之间有着什么样的关系
    import matplotlib.pyplot as plt
    # fig = plt.figure()
    # fig.set(alpha=0.2)  # 设定图表颜色alpha参数
    plt.subplot2grid((2, 3), (0, 0))             # 在一张大图里分列几个小图
    data_train.Survived.value_counts().plot(kind='bar')  # plots a bar graph of those who survived vs those who did not.
...
def getONEHOTAndDiscretization(data_train):
...
if __name__ == '__main__':
    ...
    data_train, rfr = set_missing_ages1(data_train)
    data_train = set_Cabin_type1(data_train)
    # print data_train
...
# 我们使用pandas的get_dummies来完成这个工作，并拼接在原来的data_train之上，如下所示
    df = getONEHOTAndDiscretization(data_train)
...
# 这样可以加速logistic regression的收敛
    print 'scaling zhiqian======='
    df_scaling, age_scale_param, fare_scale_param = getScaling(df)
    # 接下来咱们对训练集和测试集做一样的操作
    clf, X_input, y = procc_logisticreg(df_scaling)
    procc_testset(clf, age_scale_param, fare_scale_param)
...
# 回到我们的问题，我们用scikit-learn里面的learning_curve来帮我们分辨我们模型的状态。举个例子，这里我们一起画一下我们最先得到的baseline model的learning curve。
    ...
    plot_learning_curve(clf, u"学习曲线", X_input, y)
...
# 我们用scikit-learn里面的Bagging来完成上面的思路，过程非常简单。代码如下：
    procc_modelfusion(df_test, data_test)
    # 下面是咱们用别的分类器解决这个问题的代码：
