...
# 模型设计
from tensorflow.keras.layers import Activation, Input, Embedding, Flatten, Conv1D, MaxPooling1D, GlobalMaxPooling1D, \
...
def dssm(index2vec, max_reviews=5, dim=32, J=4):
    ...
    user_embedding = Embedding(len(index2vec), dim, weights=[index2vec], input_length=max_reviews, trainable=False)(
        user_input)
    ...
    neg_embeddings = [Embedding(len(index2vec), dim, weights=[index2vec], trainable=False)(neg_input) for neg_input in
                      neg_inputs]
...
# 开始训练
history = model.fit([user_input, pos_input] + neg_inputs, y, batch_size=1024, epochs=10, verbose=2,
...
model.fit([user_input, pos_input] + neg_inputs, y, batch_size=1024, epochs=10, verbose=0, validation_split=0.2,
          shuffle=True, callbacks=[d])
